...
- Done 2026-09-04: Caddy cut over to `web/` via same-origin `/api/*` route (verified: caddy validate, live proxy chain, prod build). Retrieval RAGAS experiments DONE (top-8 + bge A/B, both clean); bge-reranker-base now the default (`95d115a`).
- Done 2026-09-05: test-ordering wart fixed (`dc0f2aa`, 53/53); Q7/Q13 chunking item closed as stemming artifacts; docs refreshed (todos status note, Q14 tick).
- Done 2026-09-05: DEPLOYED to Lightsail (`hybrid-rag-prod`, Ubuntu 22.04, Docker CE) at https://rag.noblechicken.me (TLS via Caddy; static IP + DNS verified; prod ingest + live query verified). UI redesigned dark-first (Jost/Mulish/mono, `a526274`) — redeploy on the box pending.
- Remaining: box-side verification (restart persistence, 3-doc corpus top-up, Groq-backed prod eval re-run) + UI redeploy — all need the box owner (commands issued, see `DEPLOY.md`).
