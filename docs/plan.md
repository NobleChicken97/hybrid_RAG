...
- Pending (2026-09-04): Streamlit cutover BLOCKED on backend-URL design — `web/` parity verified (all 6 routes 200, all backend endpoints live) but `NEXT_PUBLIC_BACKEND_URL=http://backend:8000` is unreachable from browsers; needs a Caddy `/api/* → backend` route + relative URL before switching. Quota-burning retrieval experiments (`rerank_top_n` 8 re-run, bge-reranker A/B) and AWS provisioning still open.
