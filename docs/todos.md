...
- [ ] Caddy cutover BLOCKED 2026-09-04 (found during parity check, not cut over): `Dockerfile.web` bakes `NEXT_PUBLIC_BACKEND_URL=http://backend:8000`, which only resolves inside the compose network — browsers hitting the public site would fail every fetch, and `next.config.ts` has no rewrites. Correct fix is a `/api/* → backend:8000` Caddy route plus a relative/public backend URL (api.ts already supports empty `BACKEND_URL`), then re-verify before switching `reverse_proxy` off Streamlit
